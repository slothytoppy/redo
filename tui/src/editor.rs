...
impl Editor {
    pub fn draw(&mut self, frame: &mut Frame, editor_area: Rect, todos: &TodoList) {
        let list = List::new(todos.data.iter().map(|todo| todo.data.as_str()))
...
impl EventHandler for Editor {
    ...
    fn handle_event(&mut self, event: &Event, list: &Self::Input) -> Option<Self::Event> {
